...
func BrokerCheck(authRequest entity.AuthRequest) {
	...
	if err != nil {
		logger.Logger.Error("Inavlid " + string(tokens.ACCESS_TOKEN) + " Token: " + err.Error())
...
func validateRefreshToken(Answer entity.AuthAnswer, authRequest entity.AuthRequest) {
	...
	if err := tokens.VerifyTokenType(string(tokens.REFRESH_TOKEN), tokenType); err != nil {
		Answer.Err = err.Error()
		go producer.AnswerToken(Answer, authRequest.Partition)
		return
	}
	...
	logger.Logger.Info("User ID from " + string(tokens.REFRESH_TOKEN) + " Token is: " + fmt.Sprint(userId))
...
func validateAccessToken(validatedAccessToken *jwt.Token, Answer entity.AuthAnswer, authRequest entity.AuthRequest) {
	...
	if err := tokens.VerifyTokenType(string(tokens.ACCESS_TOKEN), accessTokenType); err != nil {
		logger.Logger.Error(err.Error())
		Answer.Err = err.Error()
		go producer.AnswerToken(Answer, authRequest.Partition)
		return
	...
	logger.Logger.Info("User ID from " + string(tokens.ACCESS_TOKEN) + " Token is: " + fmt.Sprint(userId))
