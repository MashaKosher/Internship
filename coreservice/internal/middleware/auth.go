...
func AuthMiddleWare() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		logger.Logger.Info("Middleware for Token check")
		accessToken, err := c.Cookie("access")
		if err != nil || len(accessToken) == 0 {
			logger.Logger.Error("No access token")
			c.JSON(http.StatusBadRequest, gin.H{"error": "No access token"})
			c.Abort()
			return
		}
		logger.Logger.Info("Access Token:" + accessToken)
		...
		if len(answer.Err) != 0 {
			logger.Logger.Error(answer.Err)
		...
		if len(answer.NewAccessToken) != 0 {
			logger.Logger.Info("New access Token")
		...
		if exists {
			logger.Logger.Info("Player already in DB: " + fmt.Sprintln(player))
		...
		logger.Logger.Info("User added successfully: " + fmt.Sprintln(player))
